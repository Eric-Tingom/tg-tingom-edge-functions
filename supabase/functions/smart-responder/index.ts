...
serve(async (req)=>{
  ...
  const { data: recentLogs, error: logError } = await supabase.from('sync_log').select(`
      *,
  ...
  .limit(10);
  if (logError) return new Response(JSON.stringify({
  ...
  for (const item of recentLogs){
    ...
    const { data: rules } = await supabase.from('operating_manual').select('content, agent_restriction').contains('keywords', [
      item.source_type
    ]) // e.g., 'ticket' or 'deal'
    .limit(3);
