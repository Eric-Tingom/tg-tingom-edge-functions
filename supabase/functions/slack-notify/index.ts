...
interface SlackPayload {
  ...
  hubspot_company_type?: string;
  hubspot_ticket_ids?: string[];
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!webhookUrl) {
      return jsonResponse({ error: "No Slack webhook URL configured" }, 500);
    }
    ...
    if (!slackRes.ok) {
      ...
      return jsonResponse({ error: `Slack send failed: ${slackRes.status} - ${errText}` }, 500);
    }
...
function buildSlackMessage(payload: SlackPayload): any {
  const emoji = getEmoji(payload.email_type, payload.priority);
  const priorityBadge = payload.priority === "urgent" ? " 🔴 URGENT" :
  ...
  if (payload.hubspot_ticket_ids && payload.hubspot_ticket_ids.length > 0) {
    const ticketLinks = payload.hubspot_ticket_ids.map(
      (id: string) => `<https://app.hubspot.com/contacts/4736045/ticket/${id}|#${id}>`
    ).join(", ");
  ...
  if (payload.body_preview) {
    blocks.push({
      type: "context",
      elements: [{ type: "mrkdwn", text: `> ${payload.body_preview.substring(0, 200)}${payload.body_preview.length > 200 ? "..." : ""}` }],
    });
  }
...
function getEmoji(emailType: string, priority: string): string {
  if (priority === "urgent") return "🚨";
  switch (emailType) {
    case "client_work_request": return "📋";
    case "client_status_update": return "💬";
    case "client_billing": return "💰";
    case "lead_inbound": return "🎯";
    case "lead_linkedin": return "🔗";
    case "lead_platform_notification": return "📣";
    case "vendor_billing": return "🧾";
    case "partner_communication": return "🤝";
    default: return "📧";
  }
}

function getTypeLabel(emailType: string): string {
  switch (emailType) {
    case "client_work_request": return "Client Work Request";
    case "client_status_update": return "Client Update";
    case "client_billing": return "Client Billing";
    case "lead_inbound": return "New Inbound Lead";
    case "lead_linkedin": return "LinkedIn Lead";
    case "lead_platform_notification": return "Platform Lead";
    case "vendor_billing": return "Vendor Bill";
    case "partner_communication": return "Partner Message";
    case "event_invitation": return "Event Invitation";
    default: return emailType.replace(/_/g, " ").replace(/\b\w/g, c => c.toUpperCase());
  }
}
