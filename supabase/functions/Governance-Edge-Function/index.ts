...
serve(async (req) => {
  ...
  const { data: assistant } = await supabase
    .from('assistants')
    .select('scope, rules_assigned, scope_confirmation_required')
    .eq('assistant_id', assistant_id)
    .single();

  // 2. GET OPERATING MANUAL RESTRICTIONS
  const { data: manualRules } = await supabase
    .from('operating_manual')
    .select('content, agent_restriction')
    .filter('id', 'in', `(${assistant.rules_assigned.join(',')})`);
  ...
  if (!isAllowed) {
    return new Response(JSON.stringify({ error: "Scope Violation Detected" }), { status: 403 });
  }
