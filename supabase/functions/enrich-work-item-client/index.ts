...
Deno.serve(async (req: Request) => {
  try {
    ...
    if (!orphanItems || orphanItems.length === 0) {
      return new Response(JSON.stringify({ message: 'No orphan work items found', enriched: 0 }), {
    ...
    if (!hubspotToken) {
      return new Response(JSON.stringify({ error: 'HubSpot token not found in vault' }), {
        status: 500,
        headers: { 'Content-Type': 'application/json' }
      });
    }
    ...
    for (const item of orphanItems) {
      try {
        ...
        const assocRes = await fetch(
          `https://api.hubapi.com/crm/v4/objects/tickets/${item.source_id}/associations/companies`,
        ...
        await supabase
          .from('work_items')
          .update({ client_name: clientName, hubspot_company_id: companyId, needs_review: false })
          .eq('id', item.id);
