...
interface BillingPeriodResult {
  ...
  action: "created" | "skipped_exists" | "skipped_no_retainer" | "error";
  hubspot_deal_id?: string;
  deal_name?: string;
  amount?: number;
  error?: string;
  prior_month_invoice_status?: string;
}
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { data: clients, error: clientErr } = await supabase
      ...
      .select("client_name, hubspot_id, billing_model, monthly_retainer_amount")
      .in("billing_model", ["flat_retainer", "capped_retainer", "monthly_retainer"])
      .eq("is_internal", false)
      .not("monthly_retainer_amount", "is", null);
    ...
    const { data: existingPeriods } = await supabase
      .from("retainer_billing_periods")
      .select("client_name, hubspot_deal_id, billing_month")
      .eq("billing_month", targetMonth);

    const existingMap = new Map(
      (existingPeriods || []).map((p: any) => [p.client_name, p.hubspot_deal_id])
    );
    ...
    const { data: priorPeriods } = await supabase
      .from("retainer_billing_periods")
      .select("client_name, invoice_status")
      .eq("billing_month", priorMonth);

    const priorInvoiceMap = new Map(
      (priorPeriods || []).map((p: any) => [p.client_name, p.invoice_status])
    );
    ...
    if (!hubspotToken && !dryRun) {
      return jsonResponse({ error: "Could not obtain HubSpot access token" }, 500);
    }

    // Step 5: Process each client
    const results: BillingPeriodResult[] = [];

    for (const client of clients as RetainerClient[]) {
      const result: BillingPeriodResult = {
          ...
          types: [
            {
              ...
              associationTypeId: 342,  // deal_to_company
            },
          ],
        },
      ],
...
function getPriorMonth(billingMonth: string): string {
  const [year, month] = billingMonth.split("-").map(Number);
  if (month === 1) return `${year - 1}-12`;
  return `${year}-${String(month - 1).padStart(2, "0")}`;
}

function getLastDayOfMonth(billingMonth: string): string {
  const [year, month] = billingMonth.split("-").map(Number);
  const lastDay = new Date(year, month, 0).getDate();
  return `${billingMonth}-${String(lastDay).padStart(2, "0")}`;
}

function buildDealName(clientName: string, billingMonth: string): string {
  const [year, month] = billingMonth.split("-").map(Number);
...
function jsonResponse(data: any, status = 200, cors = false) {
  return new Response(data ? JSON.stringify(data, null, 2) : null, {
    status,
    headers: {
      "Content-Type": "application/json",
      "Access-Control-Allow-Origin": "*",
      ...(cors ? { "Access-Control-Allow-Methods": "POST, OPTIONS", "Access-Control-Allow-Headers": "Content-Type, Authorization" } : {}),
    },
  });
}